...
const type: ElementsType = "RectangleField"
...
const propertiesSchema = z.object({
    ...
    color: z.string().min(3).default('rgb(37 99 235 / 1)'),
    label: z.string().min(3).max(100),
    helperText: z.string().min(3).max(100).default('click to edit'),
})

export const RectangleFieldFormElement: FormElement = {
    type,
...
function PropertiesComponent({
    ...
    const propertieForm = useForm<propertiesFormSchemaType>({
        ...
        mode: "onBlur",
        defaultValues: {
            length: element.extraAttributes.length,
    ...
    function applyChanges(value: propertiesFormSchemaType){
        ...
        dispatch(updateElement({
            element: {
                ...
                extraAttributes: {
                    ...value,
                    surface: `${Number.parseInt(value.length)*Number.parseInt(value.width)}`,
                    perimeter: `${(Number.parseInt(value.length + Number.parseInt(value.width)))*2}`
                }
            }
        }))
    }
                                ...
                                render={({ field }) => {
                                    return (
                                        <FormItem>
                                            ...
                                            <FormDescription>
                                            Formula. <br /> Surface = width * height
                                            </FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )
                                ...
                                render={({ field }) => {
                                    return (
                                        <FormItem>
                                            ...
                                            <FormDescription>
                                                Formula. <br /> Perimeter = (width + height) * 2
                                            </FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )
...
function DesignerComponent({
    ...
    return (
        <div
            className={cn(
                'flex border w-full h-full items-center rounded-md bg-accent/40 px-4 py-2 pointer-events-none opacity-100 justify-center',
                mouseIsOver && "opacity-30",
            )}
            style={{
                width: `${Number.parseInt(element.extraAttributes?.width)*25}px`,
                height: `${Number.parseInt(element.extraAttributes?.length)*25}px`,
